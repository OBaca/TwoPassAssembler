...
void insert_end(node** root, char* name, char macro_table[6][81]);
...
/***************************************************************************************************
            if(!strcmp(linePointer,macro_name1)) 
            {
                for(j = 0; j < 3 ; j++)
                {                                                   ***********
                    fputs(root->next->macro_lines[j-1], newfp);     not working
                }                                                   ***********
                printf("we now replace the value\n");
                continue;
            }
 ********************************************************************************************************/           
		...
		else
		{
            ...
            insert_end(&root, macro_name1, macro_table);
...
void insert_end(node** root, char* name, char macro_table[6][81])
{
    ...
    for(i = 0; i < 6; i++)
    {
        strcpy(new_node->macro_lines[i] , macro_table[i]);
        printf("%s\n", new_node->macro_lines[i]);
    }
