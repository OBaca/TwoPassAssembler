		...
		if(!skip_macro_name)
		{
			...
			else
			{
				...
				if(!( name_check(linePointer, "endm", &lettersCounter) )) 
				{
					...
					strcpy(macro_table[num_of_macro_lines],line);
					num_of_macro_lines++;
					continue;
				}

				/*create node with the macro information*/
				add_node( macro_name1, macro_table,num_of_macro_lines);
...
void add_node( char* name, char macro_table[LENGTH_MACRO][LENGTH_LINE], int num_of_macro_lines)
{
	int j;
	...
	/*copying the information to the macro node */
    	...
    	{
        	strcpy(p->macro_lines[j] , macro_table[j]);
    	}
